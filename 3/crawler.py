...
class CrawlingRobotEnvironment(environment.Environment):
    def __init__(self, crawlingRobot):
        ...
        (
            minHandAngle,
            maxHandAngle,
        ) = self.crawlingRobot.getMinAndMaxHandAngles()
        armIncrement = (maxArmAngle - minArmAngle) / (self.nArmStates - 1)
        handIncrement = (maxHandAngle - minHandAngle) / (self.nHandStates - 1)
        self.armBuckets = [
            minArmAngle + (armIncrement * i) for i in range(self.nArmStates)
        ]
        self.handBuckets = [
            minHandAngle + (handIncrement * i)
...
class CrawlingRobot:
    ...
    def moveArm(self, newArmAngle):
        ...
        disp = self.displacement(
            self.armAngle, self.handAngle, newArmAngle, self.handAngle
        )
    ...
    def displacement(
        ...
        if y < 0:
            ...
            if yOld >= 0:
                return 0.0
            return -(x - y * (xOld - x) / (yOld - y)) + math.sqrt(
                xOld * xOld + yOld * yOld
            )
    ...
    def __init__(self, canvas):
        ...
        self.ground = canvas.create_rectangle(
            0, self.groundY, self.totWidth, self.totHeight, fill="blue"
        )
