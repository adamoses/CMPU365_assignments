...
import testClasses
import textDisplay
from util import Counter, Experiences, FixedRandom, TimeoutFunction
    ...
    def executeNIterations(
        ...
        for action in actions:
            ...
            if self.comparePrettyValues(qValues, solutionDict[qValuesKey]):
                fileOutString += (
                    "Q-Values at iteration %d for action %s are correct.\n"
                    % (n, action)
                )
                fileOutString += (
                    "   Student/correct solution:\n %s\n"
                ...
                fileOutString += outString
        if checkPolicy:
            if not self.comparePrettyValues(
                policyPretty, solutionDict["policy"]
            ):
                testPass = False
        ...
        for state in states:
            ...
            for action in actions:
                ...
                if action in possibleActions:
                    qValues[action][state] = agent.computeQValueFromValues(
                        state, action
                    )
                else:
...
class ApproximateQLearningTest(testClasses.TestCase):
    ...
    def executeNExperiences(self, grades, moduleDict, solutionDict, n):
        ...
        if lastExperience is not None:
            fileOutString += (
                "Agent observed the transition (startState = %s, action = %s, endState = %s, reward = %f)\n\n"
                % lastExperience
            )
    ...
    def writeSolution(self, moduleDict, filePath):
        with open(filePath, "w") as handle:
            for n in self.numsExperiencesForDisplay:
                ...
                handle.write(
                    self.prettyValueSolutionString(
                        "weights_k_%d" % n, pp.pformat(weights)
                    )
                )
                for action in actions:
                    handle.write(
...
class QLearningTest(testClasses.TestCase):
    ...
    def executeNExperiences(
        ...
        if checkValuesAndPolicy:
            if not self.comparePrettyValues(
                valuesPretty, solutionDict["values"]
            ):
                testPass = False
...
class EpsilonGreedyTest(testClasses.TestCase):
    ...
    def testEpsilonGreedy(self, moduleDict, tolerance=0.025):
        ...
        for state in self.grid.getStates():
            ...
            empiricalEpsilon = (
                empiricalEpsilonNumerator / empiricalEpsilonDenominator
            )
...
class EvalAgentTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        self.scoreMinimum = (
            int(testDict["scoreMinimum"])
            if "scoreMinimum" in testDict
            else None
        )
        self.nonTimeoutMinimum = (
            int(testDict["nonTimeoutMinimum"])
            if "nonTimeoutMinimum" in testDict
            else None
        )
        self.winsMinimum = (
            int(testDict["winsMinimum"])
            if "winsMinimum" in testDict
            else None
        )

        self.scoreThresholds = [
            int(s) for s in testDict.get("scoreThresholds", "").split()
        ]
        self.nonTimeoutThresholds = [
            int(s) for s in testDict.get("nonTimeoutThresholds", "").split()
        ]
        self.winsThresholds = [
            int(s) for s in testDict.get("winsThresholds", "").split()
        ]
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        results = [
            ...
            gradeThreshold(
                wins, self.winsMinimum, self.winsThresholds, "wins"
            ),
        ]
...
def computePolicy(moduleDict, grid, discount):
    valueIterator = moduleDict["valueIterationAgents"].ValueIterationAgent(
...
class GridPolicyTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        if not hasattr(moduleDict["analysis"], self.parameterFn):
            self.addMessage(
                "Method not implemented: analysis.%s" % (self.parameterFn,)
            )
            return self.testFail(grades)
        ...
        for x in range(width):
            for y in range(height):
                if (
                    self.policy[x][y] in actionMap
                    and policy[(x, y)] != actionMap[self.policy[x][y]]
                ):
                    differPoint = (x, y)
                    policyPassed = False
        ...
        if not policyPassed:
            ...
            self.addMessage(
                ...
                % (
                    differPoint,
                    actionMap[self.policy[differPoint[0]][differPoint[1]]],
                )
            )
