...
class ValueEstimationAgent(Agent):
    ...
    def __init__(self, alpha=1.0, epsilon=0.05, gamma=0.8, numTraining=10):
        """
        Sets options, which can be passed in via the Pac-Man command line
        using -a alpha=0.5,...
        alpha    - learning rate
...
class ReinforcementAgent(ValueEstimationAgent):
    ...
    def getLegalActions(self, state):
        """
        Get the actions available for a given state. This is what you should
        use to obtain legal actions for a state
        """
    ...
    def __init__(
        ...
        """
        actionFn: Function which takes a state and returns the list of legal
        actions
    ...
    def final(self, state):
        ...
        if self.episodesSoFar % NUM_EPS_UPDATE == 0:
            ...
            if self.episodesSoFar <= self.numTraining:
                ...
                print(
                    "\tAverage Rewards over all training: %.2f" % (trainAvg)
                )
            else:
