...
def readDigitData(trainingSize=100, testSize=100):
    ...
    rawTrainingData = samples.loadDataFile(
        rootdata + "trainingimages",
        trainingSize,
        DIGIT_DATUM_WIDTH,
        DIGIT_DATUM_HEIGHT,
    )
    trainingLabels = samples.loadLabelsFile(
        rootdata + "traininglabels", trainingSize
    )
    ...
    validationLabels = samples.loadLabelsFile(
        rootdata + "validationlabels", TEST_SET_SIZE
    )
...
def readGiveUpData(trainingSize=100, testSize=100):
    rootdata = "pacmandata"
    rawTrainingData, trainingLabels = samples.loadPacmanData(
        rootdata + "/give_up_training.pkl", trainingSize
    )
    rawValidationData, validationLabels = samples.loadPacmanData(
        rootdata + "/give_up_validation.pkl", testSize
    )
    rawTestData, testLabels = samples.loadPacmanData(
        rootdata + "/give_up_test.pkl", testSize
    )
    trainingData = []
    validationData = []
    testData = []
...
def readCleverData(trainingSize=100, testSize=100):
    rootdata = "pacmandata"
    rawTrainingData, trainingLabels = samples.loadPacmanData(
        rootdata + "/clever_training.pkl", trainingSize
    )
    rawValidationData, validationLabels = samples.loadPacmanData(
        rootdata + "/clever_validation.pkl", testSize
    )
    rawTestData, testLabels = samples.loadPacmanData(
        rootdata + "/clever_test.pkl", testSize
    )
    trainingData = []
    validationData = []
    testData = []
...
giveUpData = readGiveUpData(1000)
cleverData = readCleverData(1000)
...
DATASETS = {
    ...
    "giveUpData": lambda: giveUpData,
    "cleverData": lambda: cleverData,
}
...
DATASETS_LEGAL_LABELS = {
    ...
    "giveUpData": ["EAST", "WEST", "NORTH", "SOUTH", "STOP"],
    "cleverData": ["EAST", "WEST", "NORTH", "SOUTH", "STOP"],
}
...
def getAccuracy(
    ...
    trainingData, trainingLabels, validationData, validationLabels, rawTrainingData, rawValidationData, testData, testLabels, rawTestData = (
        data
    )
    ...
    print(
        str(correct),
        ("correct out of " + str(len(testLabels)) + " (%.1f%%).") % (acc),
    )
...
class GradeClassifierTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        self.accuracyThresholds = [
            int(s) for s in testDict.get("accuracyThresholds", "").split()
        ]
        self.exactOutput = testDict["exactOutput"].lower() == "true"

        self.automaticTuning = (
            testDict["automaticTuning"].lower() == "true"
            if "automaticTuning" in testDict
            else None
        )
        self.max_iterations = (
            int(testDict["max_iterations"])
            if "max_iterations" in testDict
            else None
        )
        self.featureFunction = (
            testDict["featureFunction"]
            if "featureFunction" in testDict
            else "basicFeatureExtractorDigit"
        )
    ...
    def grade_classifier(self, moduleDict):
        ...
        classifierClass = getattr(
            moduleDict[self.classifierModule], self.classifierClass
        )
