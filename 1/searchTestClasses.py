...
def parseHeuristic(heuristicText):
    ...
    for line in heuristicText.split("\n"):
        ...
        if len(tokens) != 2:
            ...
            raise Exception(
                "GraphSearch heuristic specification broken at tokens:"
                + str(tokens)
            )
        state, h = tokens
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if solution not in gold_solution:
            ...
            grades.addMessage(
                "\tcorrect (reversed) solution length: %s"
                % len(gold_solution[1])
            )
            grades.addMessage(
...
class CornerProblemTest(testClasses.TestCase):
    ...
    def solution(self, search, searchAgents):
        ...
        top, right = (
            gameState.getWalls().height - 2,
            gameState.getWalls().width - 2,
        )
        missedCorners = [
            p
            for p in ((1, 1), (1, top), (right, 1), (right, top))
            if p not in visited
        ]
